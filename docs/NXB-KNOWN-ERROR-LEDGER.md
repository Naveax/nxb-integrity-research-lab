...
NXB-IRL-006 inherits the complete `NXB-ERR-001` through `NXB-ERR-022` ledger before semantic evidence promotion work continues. Generic machine signatures include `scripts/*NxbSemantic*.ps1` and `tests/Semantic*.Tests.ps1` where the corresponding error class applies. `NXB-ERR-014` rejects the semantic shared-path/shared-expected assignment pattern from Portable V1. `NXB-ERR-022` rejects both the old unguarded semantic Python expected-failure helper and direct unguarded `powercfg` stderr merges in the Part 2 semantic power experiment. The inherited 12-test ledger contract locks these repairs without changing the test count.

Part 2 pre-final review has already caught existing `NXB-ERR-006` once (`$matches` used for GUID regex results) and `NXB-ERR-015` twice (double-quoted expected-source assertions) before native publication. These are existing classes, so no new IDs were allocated. Their complete active Part 2 surfaces were repaired before a portable authority was issued.

The IRL-006 Part 1 certification must produce zero exact-tree known-error findings before its dual-runtime semantic contract and before re-running the inherited IRL-005 V5 child authority on the same exact head. Part 2 repeats the inherited scan before its dual-runtime contract and again immediately before final publication evidence.
