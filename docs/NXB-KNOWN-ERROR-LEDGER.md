...
| NXB-ERR-022 | observed-native | Windows PowerShell 5.1 native stderr merge terminates expected-failure tests | IRL-006 Portable V2 ran Python negative controls with `2>&1` while the test process inherited `$ErrorActionPreference = 'Stop'`. PS7 completed the native call and exposed exit/output, but Windows PowerShell 5.1 promoted the redirected native stderr to `RemoteException`, aborting three expected-negative tests before their assertions. | Any cross-runtime helper that intentionally captures failing native commands must scope `$ErrorActionPreference = 'Continue'` only around the native `2>&1` call, capture `$LASTEXITCODE`, restore the previous preference in `finally`, and convert captured records to strings. The semantic test helper and ledger regression lock this pattern. |
...
NXB-IRL-006 inherits the complete `NXB-ERR-001` through `NXB-ERR-022` ledger before semantic evidence promotion work continues. Generic machine signatures include `scripts/*NxbSemantic*.ps1` and `tests/Semantic*.Tests.ps1` where the corresponding error class applies. `NXB-ERR-014` rejects the semantic shared-path/shared-expected assignment pattern from Portable V1. `NXB-ERR-022` rejects a semantic Python expected-failure helper that reaches `2>&1` without first establishing the scoped ErrorAction guard. The inherited 12-test ledger contract locks both repairs without changing the test count.
