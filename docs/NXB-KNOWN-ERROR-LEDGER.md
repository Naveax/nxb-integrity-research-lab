...
NXB-IRL-006 inherits the complete `NXB-ERR-001` through `NXB-ERR-021` ledger before adding semantic evidence authority. Generic machine signatures include `scripts/*NxbSemantic*.ps1` and `tests/Semantic*.Tests.ps1` where the corresponding error class applies. `NXB-ERR-014` additionally rejects the semantic test shared-path / shared-expected variable assignment pattern that Portable V1 exposed through PSScriptAnalyzer. The inherited 12-test ledger contract verifies that semantic authority remains inside this scanner scope without changing the locked test count.
