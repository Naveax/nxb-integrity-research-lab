...
The descriptor bytes are themselves included in the signed release envelope as `update/update-descriptor.json`. Package files are separately represented in the signed artifact set as `package/<relative-path>` rows.
...
- a target sequence less than or equal to the currently installed sequence,
...
8. publishes authoritative update state,
...
The independent Python validator reconstructs the RSA PKCS#1 v1.5 SHA-256 verification with modular exponentiation and replays 16 requirements plus 12 adversarial controls.