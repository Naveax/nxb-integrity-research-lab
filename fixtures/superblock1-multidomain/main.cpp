...
std::string BuildReceipt(const Result& result, const char* status, DWORD error_code) {
    ...
    out << "{\n"
        ...
        << "    \"worker_thread_created\": " << (result.worker_thread_created ? "true" : "false") << ",\n"
        << "    \"worker_thread_joined\": " << (result.worker_thread_joined ? "true" : "false") << "\n"
        << "  },\n"
        << "  \"claims\": {\n"
...
bool RunD3D11(Result& result) {
    const wchar_t* class_name = L"NxbSuperblock1SemanticFixtureWindow";
    WNDCLASSW wc{};
    ...
    HWND hwnd = CreateWindowExW(
        0, class_name, L"NXB SUPERBLOCK 1 fixture", WS_OVERLAPPEDWINDOW,
    ...
    return SUCCEEDED(hr) && result.presents_attempted == kPresentCount && result.presents_succeeded > 0;
}
...
bool RunLoopback(Result& result) {
    ...
    server.join();
    result.worker_thread_joined = true;
    closesocket(listener);
    WSACleanup();
...
bool RunTempFile(Result& result) {
    ...
    for (std::size_t i = 0; i < payload.size(); ++i) payload[i] = static_cast<unsigned char>((i * 17u) % 251u);

    HANDLE file = CreateFileW(temp_file, GENERIC_WRITE | GENERIC_READ, 0, nullptr, CREATE_ALWAYS, FILE_ATTRIBUTE_TEMPORARY, nullptr);
    if (file == INVALID_HANDLE_VALUE) {
