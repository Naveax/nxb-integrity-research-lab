...
if len(sys.argv) != 2:
    raise SystemExit("usage: nxb-final-production-hardening-v2.py <target-root>")
root = Path(sys.argv[1]).resolve()

PREDECESSOR_ASSETS = [
    {"name":"nxb-v1.0.0-public-evidence.zip","sha256":"51caec3a66d7300ae774219f142c2a9d2351fafb613cee0c22795709b12b9d40"},
    {"name":"nxb-v1.0.0.zip","sha256":"c489ba417f1284bfcad4d0666e61fde93ab4ed8fab7fa4e8c0ef1df5c7e9ce78"},
    {"name":"package-manifest.json","sha256":"5de097246c5d0bda633a64973491b571c8383d4ca489851f2b225e6633cbf466"},
    {"name":"production-final-closure-receipt.json","sha256":"b3914161cb851a600c2d79a6f1fb877766aa8af453d2a19e03a43884758f1355"},
]

def p(path): return root / path

def read(path): return p(path).read_text(encoding="utf-8-sig")
...
def rep(path, old, new, count=1):
    ...
    n = s.count(old)
    if n != count:
        raise SystemExit(f"{path}: expected {count} matches, found {n}: {old[:120]!r}")
    write(path, s.replace(old, new))

policy_path = p("config/nxb-v1-production-release-policy.json")
policy = json.loads(policy_path.read_text(encoding="utf-8-sig"))
if policy.get("predecessor",{}).get("head") != "a4f1b242c003333b1f34b1cd54ca37cab33fbf4f":
    raise SystemExit("production predecessor head drift")
policy["predecessor"]["assets"] = PREDECESSOR_ASSETS
policy["ci"]["native_runner_name"] = "NXB-NATIVE-WPT"
policy["safety"]["require_predecessor_asset_set_exact"] = True
policy["safety"]["require_predecessor_update_smoke"] = True
policy["safety"]["require_final_release_asset_set_exact"] = True
policy_path.write_text(json.dumps(policy, indent=2) + "\n", encoding="utf-8")

script = "scripts/Invoke-NxbV1ProductionRelease.ps1"
helper_anchor = "function Test-NxbHostedReceipt {"
helper_block = r'''function Get-NxbPredecessorExpectedAssetMap {
    [CmdletBinding()]
...
'''
rep(script, helper_anchor, helper_block + helper_anchor)

old_snapshot = r'''    $assets = @($release.assets | ForEach-Object { [pscustomobject][ordered]@{ id=[long]$_.id; name=[string]$_.name; size=[long]$_.size; digest=[string]$_.digest } } | Sort-Object name)
...
new_snapshot = r'''    $assets = @($release.assets | ForEach-Object { [pscustomobject][ordered]@{ id=[long]$_.id; name=[string]$_.name; size=[long]$_.size; digest=[string]$_.digest } } | Sort-Object name)
    ...
    return [pscustomobject][ordered]@{ release_id=[long]$release.id; tag=[string]$release.tag_name; draft=[bool]$release.draft; prerelease=[bool]$release.prerelease; assets=$assets }'''
rep(script, old_snapshot, new_snapshot)
rep(script,
...
old_download = r'''$hostedArchiveSha = Save-NxbGitHubArtifactArchive -ArtifactId ([long]$hostedArtifact[0].id) -OutputPath $hostedArchive -ExpectedDigest ([string]$hostedArtifact[0].digest) -Token $ghToken
...
$ghToken = $null'''
new_download = r'''$hostedArchiveSha = Save-NxbGitHubArtifactArchive -ArtifactId ([long]$hostedArtifact[0].id) -OutputPath $hostedArchive -ExpectedDigest ([string]$hostedArtifact[0].digest) -Token $ghToken
$nativeArchiveSha = Save-NxbGitHubArtifactArchive -ArtifactId ([long]$nativeArtifact[0].id) -OutputPath $nativeArchive -ExpectedDigest ([string]$nativeArtifact[0].digest) -Token $ghToken
...
$ghToken = $null'''
rep(script, old_download, new_download)

old_stage = r'''Write-Information '[12/18] Production-signed Stage-only updater smoke'
...
new_stage = r'''Write-Information '[12/18] Frozen v1.0.0 -> production-signed v1.0.1 Stage-only updater smoke'
$updateInstallRoot = Join-Path $releaseRoot 'update-smoke-install'
$updateStateRoot = Join-Path $releaseRoot 'update-smoke-state'
[IO.Directory]::CreateDirectory($updateStateRoot) | Out-Null
$updateInstallReceipt = Join-Path $releaseRoot 'update-smoke-install-receipt.json'
...
if (Test-Path -LiteralPath $updateStateRoot) { Remove-Item -LiteralPath $updateStateRoot -Recurse -Force }'''
rep(script, old_stage, new_stage)
rep(script,
...
final_anchor = "Assert-Nxb ((Get-NxbSha256 -Path $downloadedFinal) -ceq $finalSha) 'Final closure receipt uploaded bytes do not match local canonical bytes.'"
final_extra = r'''Assert-Nxb ((Get-NxbSha256 -Path $downloadedFinal) -ceq $finalSha) 'Final closure receipt uploaded bytes do not match local canonical bytes.'
...
rep(script, final_anchor, final_extra)

validator = "tools/validate_v1_successor.py"
rep(validator,
...
production_header = '''def validate_production_release_policy(policy):
    ...
    return all(
'''
production_header_new = '''def validate_production_release_policy(policy):
    predecessor = policy.get("predecessor", {})
    implementation = policy.get("implementation", {})
    branches = policy.get("branches", {})
    ci = policy.get("ci", {})
    ...
    return all(
'''
rep(validator, production_header, production_header_new)
production_hashes = '''            predecessor.get("package_sha256") == PACKAGE_SHA256,
...
'''
production_hashes_new = '''            predecessor.get("package_sha256") == PACKAGE_SHA256,
            predecessor.get("final_closure_sha256") == FINAL_CLOSURE_SHA256,
            ...
            implementation.get("certified_head") == HISTORICAL_IMPLEMENTATION_HEAD,
'''
rep(validator, production_hashes, production_hashes_new)
rep(validator,
...
rep(validator, '"authority": "nxb-v1-successor-independent-v11"', '"authority": "nxb-v1-successor-independent-v12"')
rep(validator, '"negative_control_count": 6,', '"negative_control_count": 8,')

tests = "tests/V1Successor.Tests.ps1"
rep(tests,
...
doc = "docs/NXB-V1-PRODUCTION-RELEASE.md"
rep(doc,
...
pdoc = json.loads(policy_path.read_text())
assert pdoc["predecessor"]["assets"] == PREDECESSOR_ASSETS
assert pdoc["ci"]["native_runner_name"] == "NXB-NATIVE-WPT"
for k in ("require_predecessor_asset_set_exact","require_predecessor_update_smoke","require_final_release_asset_set_exact"):
    assert pdoc["safety"][k] is True
source = read(script)
for token in (
    "Get-NxbPredecessorExpectedAssetMap","Save-NxbGitHubReleaseAsset","Resolve-NxbExtractedPackageRoot",
    "Native runner name identity drift.","Predecessor install-state identity drift before Stage.",
    "Stage-only smoke mutated predecessor install state.","Final GitHub Release asset set drift:"
):
    assert token in source, token
assert "-Action Apply" not in source
assert len(re.findall(r"(?m)^\s*It\s+'", read(tests))) == 6
print("PATCH_OK")
