...
Describe 'SUPERBLOCK 2 L4 direct-state transition V2 contract' {
    BeforeAll {
        function Get-NxbL4V2TestRoot {
    ...
    It 'keeps active L4 V2 components repo-owned' {
        $root = Get-NxbL4V2TestRoot
        foreach ($relative in @(
            'scripts\Invoke-NxbPlatformDirectStateTransitions.ps1',
            'tools\validate_platform_direct_state_transitions.py',
            'scripts\Invoke-NxbSuperblock2DirectStateTransitionCertificationV2.ps1'
    ...
    It 'does not require PnP inventory stability for certification success' {
        $root = Get-NxbL4V2TestRoot
        $cert = Get-Content -LiteralPath (Join-Path $root 'scripts\Invoke-NxbSuperblock2DirectStateTransitionCertificationV2.ps1') -Raw
