...
function Invoke-NxbStorageReplayPesterRun {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    $tempRoot = Join-Path `
        ([IO.Path]::GetTempPath()) `
        ("nxb-storage-replay-pester-$([guid]::NewGuid().ToString('N'))")
...
if ($summary.passed -ne 6 -or $summary.failed -ne 0 -or
    $summary.skipped -ne 0 -or $summary.total -ne 6) {
    exit 1
}
'@ | Set-Content -LiteralPath $childPath -Encoding UTF8
    ...
    try {
        ...
        if ([int]$summary.passed -ne 6 -or [int]$summary.failed -ne 0 -or
            [int]$summary.skipped -ne 0 -or [int]$summary.total -ne 6) {
            throw "$Label Pester result is not clean 6/6."
        }
...
if ($env:OS -cne 'Windows_NT') {
    throw 'Storage downstream replay local validation requires Windows.'
}
if ($PSVersionTable.PSEdition -cne 'Core') {
    throw 'Storage downstream replay local validation must run in PowerShell 7.'
}
...
$replayRunnerPath = Join-Path $PSScriptRoot 'Invoke-NxbStorageDownstreamReplayValidation.ps1'
$testPath = Join-Path $repositoryRoot 'tests\StorageDownstreamReplay.Tests.ps1'
$runnerPath = $PSCommandPath
foreach ($requiredPath in @($replayRunnerPath, $testPath, $runnerPath)) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
        throw "Required storage downstream replay file missing: $requiredPath"
    }
}

foreach ($scriptPath in @($replayRunnerPath, $testPath, $runnerPath)) {
    $tokens = $null
    $errors = $null
...
$ps7 = Invoke-NxbStorageReplayPesterRun `
    -Executable $pwshPath `
    -TestPath $testPath `
    -Label 'PowerShell 7 storage downstream replay'
$ps51 = Invoke-NxbStorageReplayPesterRun `
    -Executable $windowsPowerShellPath `
    -TestPath $testPath `
    -Label 'Windows PowerShell 5.1 storage downstream replay'
...
if ($LASTEXITCODE -ne 0 -or $postDirty.Count -gt 0) {
    throw 'Storage downstream replay local validation dirtied the exact-head worktree.'
}
...
$result = [pscustomobject][ordered]@{
    ...
    semantic_equivalence_is_sufficient = $false
...
Write-Information -MessageData "Storage downstream replay local validation passed: $currentHead" -InformationAction Continue
...
Write-Information -MessageData 'Byte-identical summary required: True' -InformationAction Continue
Write-Information -MessageData 'Semantic equivalence alone sufficient: False' -InformationAction Continue
