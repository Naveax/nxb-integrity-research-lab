...
function Get-NxbPlatformEventV2Sha256Text {
    [CmdletBinding()]
...
function ConvertTo-NxbPlatformEventV2CanonicalNode {
    [CmdletBinding()]
    ...
    if ($Value -is [System.Collections.IDictionary]) {
        ...
        foreach ($key in @($Value.Keys | ForEach-Object { [string]$_ } | Sort-Object -CaseSensitive)) {
            $dictionaryResult[$key] = ConvertTo-NxbPlatformEventV2CanonicalNode -Value $Value[$key]
        }
        return $dictionaryResult
    }
    if ($Value -is [System.Collections.IEnumerable]) {
        $items = @($Value | ForEach-Object { ConvertTo-NxbPlatformEventV2CanonicalNode -Value $_ })
    ...
    foreach ($name in @($Value.PSObject.Properties.Name | Sort-Object -CaseSensitive)) {
        $objectResult[$name] = ConvertTo-NxbPlatformEventV2CanonicalNode -Value $Value.PSObject.Properties[$name].Value
    }
    return $objectResult
}

function Get-NxbPlatformEventV2OrdinalHexKey {
    [CmdletBinding()]
    param([Parameter()][AllowNull()][object]$Value)
...
function Get-NxbPlatformEventV2OrdinalUniqueStrings {
    [CmdletBinding()]
...
function Sort-NxbPlatformEventV2Definitions {
    [CmdletBinding()]
    param([Parameter()][AllowNull()][object[]]$Values)
    $items = @(
        @($Values) | Sort-Object `
            ...
            @{ Expression = { Get-NxbPlatformEventV2OrdinalHexKey -Value $_.level }; Ascending = $true },
            @{ Expression = { Get-NxbPlatformEventV2OrdinalHexKey -Value $_.task }; Ascending = $true },
            @{ Expression = { Get-NxbPlatformEventV2OrdinalHexKey -Value $_.opcode }; Ascending = $true }
    )
    Write-Output -InputObject $items -NoEnumerate
}

function Sort-NxbPlatformEventV2Shapes {
    [CmdletBinding()]
    param([Parameter()][AllowNull()][object[]]$Values)
    $items = @(
        @($Values) | Sort-Object `
...
function Sort-NxbPlatformEventV2Logs {
    [CmdletBinding()]
    param([Parameter()][AllowNull()][object[]]$Values)
    $items = @(@($Values) | Sort-Object @{ Expression = { Get-NxbPlatformEventV2OrdinalHexKey -Value $_.log_name }; Ascending = $true })
    Write-Output -InputObject $items -NoEnumerate
}

function Sort-NxbPlatformEventV2Providers {
    [CmdletBinding()]
    param([Parameter()][AllowNull()][object[]]$Values)
    $items = @(@($Values) | Sort-Object @{ Expression = { Get-NxbPlatformEventV2OrdinalHexKey -Value $_.provider_name }; Ascending = $true })
    Write-Output -InputObject $items -NoEnumerate
}

function Write-NxbPlatformEventV2Json {
    [CmdletBinding()]
    param(
...
try {
    ...
    foreach ($provider in @($snapshot.providers)) {
        foreach ($definition in @($provider.event_definitions)) {
            $definition.keywords = Get-NxbPlatformEventV2OrdinalUniqueStrings -Values @($definition.keywords)
        }
        $provider.event_definitions = Sort-NxbPlatformEventV2Definitions -Values @($provider.event_definitions)
        $provider.event_definition_count = [int]@($provider.event_definitions).Count
        foreach ($log in @($provider.logs)) {
            $log.shapes = Sort-NxbPlatformEventV2Shapes -Values @($log.shapes)
        }
        $provider.logs = Sort-NxbPlatformEventV2Logs -Values @($provider.logs)
        $provider.attached_log_count = [int]@($provider.logs).Count
    }
    $snapshot.providers = Sort-NxbPlatformEventV2Providers -Values @($snapshot.providers)
    ...
    $canonicalMetadata = ConvertTo-NxbPlatformEventV2CanonicalNode -Value $metadataMaterial
    $metadataJson = $canonicalMetadata | ConvertTo-Json -Depth 40 -Compress
    $snapshot.provider_metadata_fingerprint_sha256 = Get-NxbPlatformEventV2Sha256Text -Text $metadataJson
    Write-NxbPlatformEventV2Json -Path $OutputPath -InputObject $snapshot
}
finally {
    if (Test-Path -LiteralPath $tempPath -PathType Leaf) { Remove-Item -LiteralPath $tempPath -Force }
}

Write-Information -MessageData "NXB platform event baseline V2 written: $([IO.Path]::GetFullPath($OutputPath))" -InformationAction Continue
