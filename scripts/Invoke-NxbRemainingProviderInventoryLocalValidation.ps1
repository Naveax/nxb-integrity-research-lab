...
function Invoke-NxbRemainingProviderPesterRun {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Executable,
    ...
    $tempRoot = Join-Path `
        ([IO.Path]::GetTempPath()) `
        ('nxb-remaining-provider-pester-' + [guid]::NewGuid().ToString('N'))
...
if ($summary.passed -ne 8 -or
    $summary.total -ne 8 -or
    $summary.failed -ne 0 -or
    $summary.skipped -ne 0) {
    exit 1
}
...
if ($env:OS -cne 'Windows_NT') {
    throw 'Remaining provider inventory local validation requires Windows.'
}
if ($PSVersionTable.PSEdition -cne 'Core') {
    throw 'Remaining provider inventory local validation requires PowerShell 7.'
}
...
if ($LASTEXITCODE -ne 0 -or $dirty.Count -gt 0) {
    throw 'Remaining provider inventory local validation requires a clean exact-head worktree.'
}

$collectorPath = Join-Path $PSScriptRoot 'Invoke-NxbRemainingProviderInventory.ps1'
$testPath = Join-Path $repositoryRoot 'tests\RemainingProviderInventory.Tests.ps1'
$capabilitySchemaPath = Join-Path $repositoryRoot 'schemas\system-capabilities.schema.json'
$capabilityCollectorPath = Join-Path $PSScriptRoot 'Get-SystemCapabilities.ps1'
...
foreach ($requiredPath in @(
    ...
    $capabilityCollectorPath,
    $runnerPath
)) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
        throw "Required remaining provider validation file missing: $requiredPath"
    }
}
...
$capabilitySchema = Get-Content -LiteralPath $capabilitySchemaPath -Raw |
    ConvertFrom-Json
$requiredDomains = @($capabilitySchema.properties.domains.required)
foreach ($domain in @(
    'gpu',
    'network',
    'bus_and_devices',
    ...
    if ($requiredDomains -notcontains $domain) {
        throw "Existing capability contract is missing required domain: $domain"
    }
}
...
$ps7 = Invoke-NxbRemainingProviderPesterRun `
    -Executable $pwshPath `
    -TestPath $testPath `
    -Label 'PowerShell 7 remaining provider inventory'
$ps51 = Invoke-NxbRemainingProviderPesterRun `
    -Executable $windowsPowerShellPath `
    -TestPath $testPath `
    -Label 'Windows PowerShell 5.1 remaining provider inventory'
...
if ($LASTEXITCODE -ne 0 -or $postDirty.Count -gt 0) {
    throw 'Remaining provider inventory local validation dirtied the exact-head worktree.'
}
...
Write-Information -MessageData "Remaining provider inventory local validation passed: $currentHead" -InformationAction Continue
...
Write-Information -MessageData 'Existing capability domains: PASS' -InformationAction Continue
