...
$descriptorPath=Join-Path $fixtureRoot 'update-descriptor.json'; $descriptor=[pscustomobject][ordered]@{ schema_version=1; contract_id='nxb-v1-update-descriptor-v1'; channel='stable'; release_version=[string]$policy.target_version; release_sequence=1; release_head=$currentHead; certified_implementation_head='a10535b294c4d7ba8a4c3683154609087bf50c4b'; package_manifest_sha256=$targetManifestSha; created_utc=[DateTime]::UtcNow.ToString('o') }; [IO.File]::WriteAllText($descriptorPath,(($descriptor|ConvertTo-Json -Depth 6)+[Environment]::NewLine),[Text.UTF8Encoding]::new($false))
...
try {
    ...
    $envelope=ConvertTo-NxbV1SignedReleaseEnvelope -Signer $signer -ReleaseHead $currentHead -CertifiedImplementationHead 'a10535b294c4d7ba8a4c3683154609087bf50c4b' -PackageManifestSha256 $targetManifestSha -ReleaseNotesSha256 $emptyNotesSha -Artifacts @($artifactRows) -ReleaseVersion ([string]$policy.target_version)
