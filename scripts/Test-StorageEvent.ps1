...
if (-not $python) {
    throw 'Python not found. Storage event validation requires Python and jsonschema.'
}
...
if (-not (Test-Path -LiteralPath $validator -PathType Leaf)) {
    throw "Storage event semantic validator not found: $validator"
}
...
if ($exitCode -ne 0) {
    throw (
        "Storage event validation failed (exit $exitCode):`n" +
