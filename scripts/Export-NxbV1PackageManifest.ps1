...
$packagePrefix = $packageFull.TrimEnd($trimChars) + [IO.Path]::DirectorySeparatorChar
if ([string]::Equals($outputFull,$packageFull,[StringComparison]::OrdinalIgnoreCase) -or $outputFull.StartsWith($packagePrefix,[StringComparison]::OrdinalIgnoreCase)) {
    throw 'Package manifest output must be outside PackageRoot.'
}
