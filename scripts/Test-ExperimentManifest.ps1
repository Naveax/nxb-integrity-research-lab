...
param(
    ...
    [string]$SchemaPath = (Join-Path (Split-Path -Parent $PSScriptRoot) 'schemas\experiment.schema.json')
)
...
if (-not (Test-Path -LiteralPath $SchemaPath -PathType Leaf)) {
    throw "Schema bulunamadı: $SchemaPath"
}
...
if (-not (Test-Path -LiteralPath $validator -PathType Leaf)) {
    throw "Manifest validator bulunamadı: $validator"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Manifest schema doğrulaması başarısız:`n$($output -join [Environment]::NewLine)"
}
