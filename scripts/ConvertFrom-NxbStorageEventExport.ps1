...
function Resolve-NxbStorageEventPython {
    [CmdletBinding()]
    param()
...
if ([string]::IsNullOrWhiteSpace($PSCommandPath) -or
    ...
    throw 'Storage ETL adapter provenance path could not be resolved.'
}

$repositoryRoot = Split-Path -Parent $PSScriptRoot
$pythonConverter = Join-Path $repositoryRoot 'tools\convert_storage_event_export.py'
$validatorPath = Join-Path $PSScriptRoot 'Test-StorageEtlSummary.ps1'
...
foreach ($requiredPath in @($pythonConverter, $validatorPath, $schemaPath)) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
        throw "Storage ETL adapter input not found: $requiredPath"
    }
}

$pythonPath = Resolve-NxbStorageEventPython
if ([string]::IsNullOrWhiteSpace($pythonPath)) {
    throw 'Python not found. Storage event export conversion requires Python.'
}
...
$arguments = @(
    ...
    '--target-process-id', [string]$TargetProcessId,
    '--target-process-start-utc',
    $TargetProcessStartUtc.ToUniversalTime().ToString(
        "yyyy-MM-ddTHH:mm:ss.ffffff'Z'",
        [Globalization.CultureInfo]::InvariantCulture
    ),
    '--target-image-sha256', $TargetImageSha256,
    '--trace-loss', $TraceLoss,
    '--circular-overwrite', $CircularOverwrite,
    '--parser-completeness', $ParserCompleteness,
...
try {
    if ($exitCode -ne 0) {
        $message = @($output | ForEach-Object { [string]$_ }) -join `
            [Environment]::NewLine
        throw (
            "Storage event export conversion failed (exit $exitCode):`n" +
            $message
        )
    }
