...
function Test-NxbL4V2Administrator {
    [CmdletBinding()]
    param()
...
function Invoke-NxbL4V2Pester {
    ...
    $tempRoot = Join-Path ([IO.Path]::GetTempPath()) ("nxb-l4v2-pester-$([guid]::NewGuid().ToString('N'))")
...
function Invoke-NxbL4V2PythonValidation {
    [CmdletBinding()]
    param(
    ...
    if ($exitCode -ne 0) { throw "L4 V2 Python validation failed: exit=$exitCode" }
    return (Get-Content -LiteralPath $OutputPath -Raw | ConvertFrom-Json)
}

if ($env:OS -cne 'Windows_NT') { throw 'SUPERBLOCK 2 L4 V2 certification requires Windows.' }
if ($PSVersionTable.PSEdition -cne 'Core') { throw 'SUPERBLOCK 2 L4 V2 certification requires PowerShell 7.' }
if (-not (Test-NxbL4V2Administrator)) { throw 'SUPERBLOCK 2 L4 V2 certification requires elevated PowerShell 7.' }

$repositoryRoot = Split-Path -Parent $PSScriptRoot
$git = (Get-Command git.exe -ErrorAction Stop).Source
$currentHead = (& $git -C $repositoryRoot rev-parse HEAD).Trim().ToLowerInvariant()
if ($LASTEXITCODE -ne 0 -or $currentHead -cne $ExpectedHead.ToLowerInvariant()) { throw "L4 V2 exact-head mismatch. Expected=$ExpectedHead actual=$currentHead" }
$dirty = @(& $git -C $repositoryRoot status --porcelain=v1 --untracked-files=all)
if ($LASTEXITCODE -ne 0 -or $dirty.Count -gt 0) { throw 'L4 V2 certification requires a clean exact-head worktree.' }

$outputFull = [IO.Path]::GetFullPath($OutputDirectory)
$repositoryPrefix = [IO.Path]::GetFullPath($repositoryRoot).TrimEnd('\') + '\'
if ($outputFull.StartsWith($repositoryPrefix,[StringComparison]::OrdinalIgnoreCase)) { throw 'L4 V2 output must remain outside the repository.' }
...
$testPath = Join-Path $repositoryRoot 'tests\PlatformDirectStateTransitionsV2.Tests.ps1'
...
foreach ($requiredPath in @($runtimePath,$testPath,$validatorPath,$PSCommandPath)) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) { throw "L4 V2 component missing: $requiredPath" }
}

Write-Information -MessageData '=== NXB IRL-004 SUPERBLOCK 2 L4 DIRECT-STATE TRANSITION CERTIFICATION V2 ===' -InformationAction Continue
...
if ($findings.Count -gt 0) { throw ("L4 V2 PSScriptAnalyzer findings: $($findings.Count)`n" + (@($findings | ForEach-Object { '{0}:{1} {2} {3}' -f $_.ScriptName,$_.Line,$_.RuleName,$_.Message }) -join "`n")) }
...
if ($LASTEXITCODE -ne 0) { throw 'L4 V2 Python syntax check failed.' }
...
try {
    ...
    $ps7Summary = Invoke-NxbL4V2Pester -Executable $pwshPath -TestPath $testPath -ExpectedCount 20 -Label 'PowerShell 7 L4 V2'
    $ps51Summary = Invoke-NxbL4V2Pester -Executable $ps51Path -TestPath $testPath -ExpectedCount 20 -Label 'Windows PowerShell 5.1 L4 V2'
}
finally {
...
$l3Receipt = Get-NxbL4V2ZipJson -ZipPath $l3Full -EntryName 'transition-surface-certification-receipt.json'
...
$validation = Invoke-NxbL4V2PythonValidation -PythonPath $pythonCommand.Source -ToolPath $validatorPath -InputPath $observationPath -OutputPath $validationPath
if ([string]$validation.status -cne 'passed') { throw 'L4 V2 validation status is not passed.' }
if (-not [bool]$validation.power_policy_transition_mapping) { throw 'L4 V2 direct power-policy mapping was not validated.' }

Write-Information -MessageData '[5/6] Conservative L4 V2 certification receipt' -InformationAction Continue
...
Write-NxbL4V2Json -Path $receiptPath -InputObject $receipt
...
Write-Information -MessageData ("SUPERBLOCK 2 L4 V2 passed: pnp_stable={0} power_mapping={1}" -f $result.pnp_inventory_stable_both,$result.power_policy_transition_mapping) -InformationAction Continue
