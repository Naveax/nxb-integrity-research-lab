...
function Test-NxbL3V2Administrator {
    [CmdletBinding()]
    param()
...
function Write-NxbL3V2Json {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][ValidateNotNullOrEmpty()][string]$Path,
        [Parameter(Mandatory)][object]$InputObject
    )
    [IO.File]::WriteAllText([IO.Path]::GetFullPath($Path),(($InputObject | ConvertTo-Json -Depth 40) + [Environment]::NewLine),[Text.UTF8Encoding]::new($false))
}

function Invoke-NxbL3V2Pester {
    [CmdletBinding()]
    param(
    ...
    $tempRoot = Join-Path ([IO.Path]::GetTempPath()) ("nxb-l3v2-pester-$([guid]::NewGuid().ToString('N'))")
...
if ($env:OS -cne 'Windows_NT') { throw 'SUPERBLOCK 2 L3 V2 certification requires Windows.' }
if ($PSVersionTable.PSEdition -cne 'Core') { throw 'SUPERBLOCK 2 L3 V2 certification requires PowerShell 7.' }
if (-not (Test-NxbL3V2Administrator)) { throw 'SUPERBLOCK 2 L3 V2 certification requires elevated PowerShell 7.' }

$repositoryRoot = Split-Path -Parent $PSScriptRoot
$git = (Get-Command git.exe -ErrorAction Stop).Source
$currentHead = (& $git -C $repositoryRoot rev-parse HEAD).Trim().ToLowerInvariant()
if ($LASTEXITCODE -ne 0 -or $currentHead -cne $ExpectedHead.ToLowerInvariant()) { throw "L3 V2 exact-head mismatch. Expected=$ExpectedHead actual=$currentHead" }
$dirty = @(& $git -C $repositoryRoot status --porcelain=v1 --untracked-files=all)
if ($LASTEXITCODE -ne 0 -or $dirty.Count -gt 0) { throw 'L3 V2 certification requires a clean exact-head worktree.' }

$outputFull = [IO.Path]::GetFullPath($OutputDirectory)
$repositoryPrefix = [IO.Path]::GetFullPath($repositoryRoot).TrimEnd('\') + '\'
if ($outputFull.StartsWith($repositoryPrefix,[StringComparison]::OrdinalIgnoreCase)) { throw 'L3 V2 output must remain outside the repository.' }
...
$discoveryScript = Join-Path $PSScriptRoot 'Get-NxbTransitionSurfaceDiscovery.ps1'
$replayScript = Join-Path $PSScriptRoot 'Invoke-NxbPlatformSurfaceDiscoveryReplay.ps1'
$testPath = Join-Path $repositoryRoot 'tests\PlatformTransitionSurfaceDiscoveryV2.Tests.ps1'
...
foreach ($requiredPath in @($discoveryScript,$replayScript,$testPath,$validatorPath,$analysisToolPath,$PSCommandPath)) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) { throw "L3 V2 component missing: $requiredPath" }
}

Write-Information -MessageData '=== NXB IRL-004 SUPERBLOCK 2 L3 TRANSITION SURFACE DISCOVERY V2 ===' -InformationAction Continue
Write-Information -MessageData '[1/7] Parser/analyzer + dual-runtime 20-test contract' -InformationAction Continue
$analyzerPaths = @($discoveryScript,$replayScript,$testPath,$PSCommandPath)
...
if ($findings.Count -gt 0) { throw ("L3 V2 PSScriptAnalyzer findings: $($findings.Count)`n" + (@($findings | ForEach-Object { '{0}:{1} {2} {3}' -f $_.ScriptName,$_.Line,$_.RuleName,$_.Message }) -join "`n")) }
...
if ($LASTEXITCODE -ne 0) { throw 'L3 V2 Python syntax check failed.' }
...
try {
    ...
    $ps7 = Invoke-NxbL3V2Pester -Executable $pwshPath -TestPath $testPath -ExpectedCount 20 -Label 'PowerShell 7 L3 V2'
    $ps51 = Invoke-NxbL3V2Pester -Executable $ps51Path -TestPath $testPath -ExpectedCount 20 -Label 'Windows PowerShell 5.1 L3 V2'
}
finally {
...
$l2Receipt = Get-NxbL3V2ZipJson -ZipPath $l2Full -EntryName 'platform-transition-certification-receipt.json'
...
Write-Information -MessageData '[3/7] Collect + independently validate discovery A/B' -InformationAction Continue
...
& $discoveryScript -OutputPath $discoveryAPath -BindingFingerprintSha256 $ExpectedBindingFingerprint -ProviderMetadataFingerprintSha256 $ExpectedProviderMetadataFingerprint | Out-Null
$validationA = Invoke-NxbL3V2Python -PythonPath $pythonCommand.Source -ToolPath $validatorPath -InputPath $discoveryAPath -OutputPath $validationAPath -Label 'L3 discovery A validation'
Start-Sleep -Milliseconds 250
& $discoveryScript -OutputPath $discoveryBPath -BindingFingerprintSha256 $ExpectedBindingFingerprint -ProviderMetadataFingerprintSha256 $ExpectedProviderMetadataFingerprint | Out-Null
$validationB = Invoke-NxbL3V2Python -PythonPath $pythonCommand.Source -ToolPath $validatorPath -InputPath $discoveryBPath -OutputPath $validationBPath -Label 'L3 discovery B validation'
...
& $replayScript -OutputPath $observationPath -DiscoveryPath $discoveryAPath -L2ReviewZipPath $l2Full -BindingFingerprintSha256 $ExpectedBindingFingerprint -ProviderMetadataFingerprintSha256 $ExpectedProviderMetadataFingerprint | Out-Null
...
$analysisA = Invoke-NxbL3V2Python -PythonPath $pythonCommand.Source -ToolPath $analysisToolPath -InputPath $observationPath -OutputPath $analysisPath -Label 'L3 differential analysis A'
Invoke-NxbL3V2Python -PythonPath $pythonCommand.Source -ToolPath $analysisToolPath -InputPath $observationPath -OutputPath $analysisReplayPath -Label 'L3 differential analysis B' | Out-Null
...
Write-Information -MessageData '[6/7] Conservative L3 receipt' -InformationAction Continue
...
Write-NxbL3V2Json -Path $receiptPath -InputObject $receipt
...
Write-Information -MessageData ("SUPERBLOCK 2 L3 V2 passed: providers={0} usable={1} pnp_candidates={2} power_candidates={3}" -f $validationA.provider_count,$validationA.usable_surface_count,$analysisA.pnp.repeated_positive_delta_shape_count,$analysisA.power.repeated_positive_delta_shape_count) -InformationAction Continue
