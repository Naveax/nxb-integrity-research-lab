...
if (-not (Test-Path -LiteralPath $schemaFull -PathType Leaf)) {
    throw "Storage ETL summary schema not found: $schemaFull"
}
...
if (-not $python) {
    throw 'Python not found. Storage ETL summary validation requires Python and jsonschema.'
}
...
if (-not (Test-Path -LiteralPath $validator -PathType Leaf)) {
    throw "Storage ETL semantic validator not found: $validator"
}
...
if ($exitCode -ne 0) {
    throw (
        "Storage ETL summary validation failed (exit $exitCode):`n" +
