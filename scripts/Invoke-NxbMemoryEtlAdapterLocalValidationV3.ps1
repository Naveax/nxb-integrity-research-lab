...
function Test-NxbMemoryEtlV3Administrator {
    [CmdletBinding()]
    param()
...
function Test-NxbMemoryEtlV3NUnit {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$Path,
...
function Get-NxbMemoryEtlV3Gate {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [object]$Summary,
...
if ($env:OS -cne 'Windows_NT') {
    throw 'Memory ETL adapter V3 validation requires real Windows.'
}
if ($PSVersionTable.PSEdition -cne 'Core') {
    throw 'Memory ETL adapter V3 validation must run in PowerShell 7.'
}
if (-not (Test-NxbMemoryEtlV3Administrator)) {
    throw 'Memory ETL adapter V3 validation requires elevated PowerShell 7.'
}
...
if ($LASTEXITCODE -ne 0 -or $workingTree.Count -gt 0) {
    throw 'Exact-head V3 validation requires a clean working tree.'
}
...
foreach ($requiredPath in @($v1Runner, $settingsPath, $PSCommandPath)) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) {
        throw "Memory ETL V3 validation input not found: $requiredPath"
    }
}
...
if ([string]::IsNullOrWhiteSpace($ResultsRoot)) {
    ...
    $ResultsRoot = Join-Path `
        ([IO.Path]::GetTempPath()) `
        "nxb-memory-etl-adapter-v3-$($currentHead.Substring(0, 12))-$stamp"
}
...
$summaryPath = Join-Path $resultsFull 'memory-etl-adapter-v3-validation-summary.json'
...
$expectedGateNames = @(
    ...
    'memory-etl-contract',
    'pester-memory-etl-pwsh',
    'pester-memory-etl-ps51'
)
if (@($rawSummary.gates).Count -ne $expectedGateNames.Count) {
    throw 'Raw ETL validation gate count mismatch.'
}
foreach ($gateEntry in @($rawSummary.gates)) {
    if ($expectedGateNames -cnotcontains [string]$gateEntry.name) {
        throw "Unexpected raw ETL gate: $($gateEntry.name)"
    }
}

$requiredPassedGates = $expectedGateNames[0..4]
$finalGates = [Collections.Generic.List[object]]::new()
foreach ($name in $requiredPassedGates) {
    $gate = Get-NxbMemoryEtlV3Gate -Summary $rawSummary -Name $name
...
$ps7Result = Test-NxbMemoryEtlV3NUnit -Path $ps7Xml -ExpectedTotal 18
$ps51Result = Test-NxbMemoryEtlV3NUnit -Path $ps51Xml -ExpectedTotal 18

$ps7Gate = Get-NxbMemoryEtlV3Gate -Summary $rawSummary -Name 'pester-memory-etl-pwsh'
$ps51Gate = Get-NxbMemoryEtlV3Gate -Summary $rawSummary -Name 'pester-memory-etl-ps51'
...
if ($selfFindings.Count -gt 0) {
    throw (
        'Memory ETL V3 runner PSScriptAnalyzer findings: ' +
        ($selfFindings |
...
$finalGates.Add([pscustomobject][ordered]@{
    name = 'memory-etl-v3-runner-psscriptanalyzer'
    status = 'passed'
    exit_code = 0
...
Write-Host "Memory ETL adapter V3 validation summary: $summaryPath"
Write-Host "Review ZIP: $reviewZip"
Write-Host 'Memory ETL adapter exact-head validation V3 completed.'
