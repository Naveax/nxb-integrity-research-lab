...
function Invoke-NxbDownstreamPesterRun {
    [CmdletBinding()]
    param(
    ...
    $tempRoot = Join-Path ([IO.Path]::GetTempPath()) ("nxb-superblock-downstream-pester-$([guid]::NewGuid().ToString('N'))")
...
if ($env:OS -cne 'Windows_NT') { throw 'SUPERBLOCK downstream validation requires Windows.' }
if ($PSVersionTable.PSEdition -cne 'Core') { throw 'SUPERBLOCK downstream validation requires PowerShell 7.' }
...
if ($LASTEXITCODE -ne 0 -or $dirty.Count -gt 0) {
    throw 'SUPERBLOCK downstream validation requires a clean exact-head worktree.'
}
...
$ps7Result = Invoke-NxbDownstreamPesterRun -Executable $pwsh -TestPath $testPath -Label 'PowerShell 7 downstream normalizer'
$ps51Result = Invoke-NxbDownstreamPesterRun -Executable $ps51 -TestPath $testPath -Label 'Windows PowerShell 5.1 downstream normalizer'

$postDirty = @(& $git.Source -C $repositoryRoot status --porcelain=v1 --untracked-files=all)
if ($LASTEXITCODE -ne 0 -or $postDirty.Count -gt 0) { throw 'Downstream validation dirtied the exact-head worktree.' }
...
Write-Information -MessageData "SUPERBLOCK downstream local validation passed: $currentHead" -InformationAction Continue
