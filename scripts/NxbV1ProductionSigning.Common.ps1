...
function Test-NxbV1SigningArtifactPath {
    ...
    if ($Path.IndexOf('|',[StringComparison]::Ordinal) -ge 0 -or
        $Path.IndexOf("`r",[StringComparison]::Ordinal) -ge 0 -or
        $Path.IndexOf("`n",[StringComparison]::Ordinal) -ge 0 -or
        $Path.IndexOf('\',[StringComparison]::Ordinal) -ge 0) { return $false }
...
function Get-NxbV1SigningPublicKey {
    ...
    $fingerprintMaterial = @(
        ...
        ('modulus_b64={0}' -f $modulusB64)
        ('exponent_b64={0}' -f $exponentB64)
    ) -join "`n"
    $fingerprint = Get-NxbV1SigningSha256Text -Text $fingerprintMaterial
...
function Get-NxbV1ReleaseCanonicalMaterial {
    ...
    if ([string]$Envelope.canonical_contract_id -cne 'nxb-v1-release-signature-canonical-v1') {
        throw 'Release signature canonical contract drift.'
    }
    ...
    $lines.Add('nxb-v1-release-signature-canonical-v1')
    $lines.Add('schema_version=1')
    $lines.Add(('release_version={0}' -f [string]$Envelope.release_version))
    $lines.Add(('release_head={0}' -f [string]$Envelope.release_head))
    $lines.Add(('certified_implementation_head={0}' -f [string]$Envelope.certified_implementation_head))
    $lines.Add(('package_manifest_sha256={0}' -f [string]$Envelope.package_manifest_sha256))
    $lines.Add(('release_notes_sha256={0}' -f [string]$Envelope.release_notes_sha256))
    $lines.Add(('artifact_count={0}' -f $paths.Count))
    $lines.Add(('signer_mode={0}' -f [string]$Envelope.signer_mode))
    $lines.Add(('signer_key_id={0}' -f [string]$Envelope.signer_key_id))
    $lines.Add(('signing_algorithm={0}' -f [string]$Envelope.signing_algorithm))
    $lines.Add(('key_size_bits={0}' -f [int]$Envelope.key_size_bits))
    $lines.Add(('public_modulus_b64={0}' -f [string]$Envelope.public_key.modulus_b64))
    $lines.Add(('public_exponent_b64={0}' -f [string]$Envelope.public_key.exponent_b64))
    $lines.Add(('public_fingerprint={0}' -f [string]$Envelope.public_key.fingerprint))
    $lines.Add(('created_utc={0}' -f [string]$Envelope.created_utc))
    foreach ($path in $paths) {
        $artifact = $artifactMap[$path]
        $lines.Add(('artifact={0}|{1}|{2}' -f $path,[int64]$artifact.bytes,[string]$artifact.sha256))
    }
    return ($lines -join "`n")
}
...
function Test-NxbV1SigningRsaProtected {
    ...
    if ($Rsa -is [Security.Cryptography.RSACng]) {
        ...
        $allowExport = [Security.Cryptography.CngExportPolicies]::AllowExport
        $allowPlain = [Security.Cryptography.CngExportPolicies]::AllowPlaintextExport
        return (($policy -band $allowExport) -eq 0 -and ($policy -band $allowPlain) -eq 0)
    }
...
function New-NxbV1CertificationSigner {
    ...
    try {
        ...
        return [pscustomobject][ordered]@{
            mode = 'certification-ephemeral'
            key_id = ('cert-ephemeral:{0}' -f $publicKey.fingerprint)
            rsa = $rsa
            public_key = $publicKey
            key_size_bits = [int]$rsa.KeySize
...
function Get-NxbV1ProductionCertificateSigner {
    ...
    try {
        ...
        $matches = @($store.Certificates | Where-Object { $_.Thumbprint.Replace(' ','').ToUpperInvariant() -ceq $normalizedThumbprint })
        if ($matches.Count -ne 1) { throw ('Production signer certificate count must be exactly one. actual={0}' -f $matches.Count) }
        ...
        $publicKey = Get-NxbV1SigningPublicKey -Rsa $rsa
        return [pscustomobject][ordered]@{
            mode = 'production-windows-certificate-store'
            key_id = ('win-cert:{0}/{1}/{2}' -f $StoreLocation,$StoreName,$normalizedThumbprint)
            rsa = $rsa
            public_key = $publicKey
            key_size_bits = [int]$rsa.KeySize
...
function New-NxbV1SignedReleaseEnvelope {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][object]$Signer,
        [Parameter(Mandatory)][string]$ReleaseHead,
        [Parameter(Mandatory)][string]$CertifiedImplementationHead,
        [Parameter(Mandatory)][string]$PackageManifestSha256,
        [Parameter(Mandatory)][string]$ReleaseNotesSha256,
    ...
    $envelope = [pscustomobject][ordered]@{
        schema_version = 1
        status = 'signed'
        canonical_contract_id = 'nxb-v1-release-signature-canonical-v1'
        release_version = '1.0.0'
        release_head = $ReleaseHead.ToLowerInvariant()
        certified_implementation_head = $CertifiedImplementationHead.ToLowerInvariant()
        package_manifest_sha256 = $PackageManifestSha256.ToLowerInvariant()
        release_notes_sha256 = $ReleaseNotesSha256.ToLowerInvariant()
        artifacts = @($Artifacts)
        signer_mode = [string]$Signer.mode
        signer_key_id = [string]$Signer.key_id
        signing_algorithm = 'RSA-PKCS1-SHA256'
        key_size_bits = [int]$Signer.key_size_bits
        public_key = $Signer.public_key
        created_utc = $CreatedUtc
        canonical_sha256 = ''
        signature_b64 = ''
        private_key_persisted = [bool]$Signer.private_key_persisted
        production_signer_claimed = [bool]$Signer.production_signer_claimed
    }
    ...
    $signature = $Signer.rsa.SignData(
        [Text.UTF8Encoding]::new($false).GetBytes($material),
...
function Test-NxbV1SignedReleaseEnvelope {
    ...
    try {
        $material = Get-NxbV1ReleaseCanonicalMaterial -Envelope $Envelope
        $canonicalSha = Get-NxbV1SigningSha256Text -Text $material
        if ($canonicalSha -cne [string]$Envelope.canonical_sha256) { return $false }
