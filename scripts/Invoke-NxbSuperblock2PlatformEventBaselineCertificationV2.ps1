...
function Test-NxbPlatformEventV2Administrator {
    [CmdletBinding()]
    param()
...
function Write-NxbPlatformEventV2CertificationJson {
    [CmdletBinding()]
    param(
...
function Invoke-NxbPlatformEventV2Pester {
    [CmdletBinding()]
    param(
    ...
    $tempRoot = Join-Path ([IO.Path]::GetTempPath()) ("nxb-platform-event-v2-pester-$([guid]::NewGuid().ToString('N'))")
...
function Get-NxbPlatformEventV2ZipJson {
    [CmdletBinding()]
    param(
...
function Invoke-NxbPlatformEventV2Validation {
    [CmdletBinding()]
    param(
    ...
    if ($exitCode -ne 0) { throw "Platform event V2 validation failed: exit=$exitCode" }
    return (Get-Content -LiteralPath $OutputPath -Raw | ConvertFrom-Json)
}

if ($env:OS -cne 'Windows_NT') { throw 'SUPERBLOCK 2 L1 V2 certification requires Windows.' }
if ($PSVersionTable.PSEdition -cne 'Core') { throw 'SUPERBLOCK 2 L1 V2 certification requires PowerShell 7.' }
if (-not (Test-NxbPlatformEventV2Administrator)) { throw 'SUPERBLOCK 2 L1 V2 certification requires elevated PowerShell 7.' }
...
if ($LASTEXITCODE -ne 0 -or $currentHead -cne $ExpectedHead.ToLowerInvariant()) {
    throw "Platform event V2 exact-head mismatch. Expected=$ExpectedHead actual=$currentHead"
}
$dirty = @(& $git -C $repositoryRoot status --porcelain=v1 --untracked-files=all)
if ($LASTEXITCODE -ne 0 -or $dirty.Count -gt 0) { throw 'Platform event V2 certification requires a clean exact-head worktree.' }

$outputFull = [IO.Path]::GetFullPath($OutputDirectory)
$repositoryPrefix = [IO.Path]::GetFullPath($repositoryRoot).TrimEnd('\') + '\'
if ($outputFull.StartsWith($repositoryPrefix,[StringComparison]::OrdinalIgnoreCase)) { throw 'L1 V2 output must remain outside the repository.' }
...
$collectorV1Path = Join-Path $PSScriptRoot 'Get-NxbPlatformEventBaseline.ps1'
$collectorPath = Join-Path $PSScriptRoot 'Get-NxbPlatformEventBaselineV3.ps1'
$coreTestPath = Join-Path $repositoryRoot 'tests\PlatformEventBaseline.Tests.ps1'
$ordinalTestPath = Join-Path $repositoryRoot 'tests\PlatformEventBaselineOrdinal.Tests.ps1'
$validatorPath = Join-Path $repositoryRoot 'tools\validate_platform_event_baseline.py'
$requiredPaths = @($collectorV1Path,$collectorPath,$coreTestPath,$ordinalTestPath,$validatorPath,$PSCommandPath)
foreach ($requiredPath in $requiredPaths) {
    if (-not (Test-Path -LiteralPath $requiredPath -PathType Leaf)) { throw "L1 V2 component missing: $requiredPath" }
}

Write-Information -MessageData '=== NXB IRL-004 SUPERBLOCK 2 L1 PLATFORM EVENT BASELINE V2 ===' -InformationAction Continue
Write-Information -MessageData '[1/7] Parser/analyzer + dual-runtime 28-test contract' -InformationAction Continue
$analyzerPaths = @($collectorV1Path,$collectorPath,$coreTestPath,$ordinalTestPath,$PSCommandPath)
...
if ($findings.Count -gt 0) {
    throw ("L1 V2 PSScriptAnalyzer findings: $($findings.Count)`n" + (@($findings | ForEach-Object { '{0}:{1} {2} {3}' -f $_.ScriptName,$_.Line,$_.RuleName,$_.Message }) -join "`n"))
}
...
if ($LASTEXITCODE -ne 0) { throw 'L1 V2 Python syntax check failed.' }
...
try {
    ...
    $ps7Core = Invoke-NxbPlatformEventV2Pester -Executable $pwshPath -TestPath $coreTestPath -ExpectedCount 20 -Label 'PowerShell 7 L1 core'
    $ps7Ordinal = Invoke-NxbPlatformEventV2Pester -Executable $pwshPath -TestPath $ordinalTestPath -ExpectedCount 8 -Label 'PowerShell 7 L1 ordinal'
    $ps51Core = Invoke-NxbPlatformEventV2Pester -Executable $ps51Path -TestPath $coreTestPath -ExpectedCount 20 -Label 'Windows PowerShell 5.1 L1 core'
    $ps51Ordinal = Invoke-NxbPlatformEventV2Pester -Executable $ps51Path -TestPath $ordinalTestPath -ExpectedCount 8 -Label 'Windows PowerShell 5.1 L1 ordinal'
}
finally {
...
$ps7Total = [int]$ps7Core.passed + [int]$ps7Ordinal.passed
$ps51Total = [int]$ps51Core.passed + [int]$ps51Ordinal.passed
if ($ps7Total -ne 28 -or $ps51Total -ne 28) { throw "L1 V2 combined test count mismatch: ps7=$ps7Total ps51=$ps51Total" }
...
$l0Receipt = Get-NxbPlatformEventV2ZipJson -ZipPath $l0Full -EntryName 'platform-binding-certification-receipt.json'
...
Write-Information -MessageData '[3/7] Collect and validate ordinal-normalized platform event baseline A' -InformationAction Continue
...
$validationA = Invoke-NxbPlatformEventV2Validation -PythonPath $pythonCommand.Source -ValidatorPath $validatorPath -InputPath $baselineAPath -OutputPath $validationAPath

Start-Sleep -Milliseconds 500
Write-Information -MessageData '[4/7] Collect and validate ordinal-normalized platform event baseline B' -InformationAction Continue
...
$validationB = Invoke-NxbPlatformEventV2Validation -PythonPath $pythonCommand.Source -ValidatorPath $validatorPath -InputPath $baselineBPath -OutputPath $validationBPath
...
if ($metadataA -cne $metadataB) { throw 'provider metadata fingerprint changed between ordinal baselines' }
if ($metadataA -notmatch '^[0-9a-f]{64}$') { throw 'provider metadata fingerprint malformed.' }
if ([int]$validationA.provider_count -ne 8 -or [int]$validationB.provider_count -ne 8) { throw 'L1 V2 provider count is not exactly eight.' }
if ([int]$validationA.event_definition_count -le 0) { throw 'L1 V2 provider event-definition inventory is empty.' }
if ([int]$validationA.attached_log_count -le 0) { throw 'L1 V2 attached-log inventory is empty.' }
if ([int]$validationA.available_log_query_count -le 0) { throw 'L1 V2 has no readable recent-event log query.' }

Write-Information -MessageData '[6/7] Conservative L1 V2 certification receipt' -InformationAction Continue
$receipt = [pscustomobject][ordered]@{
    schema_version = 2
    status = 'passed'
    head_sha = $currentHead
    static_validation = [ordered]@{
        ps7 = [ordered]@{ passed=$ps7Total; total=28 }
        ps51 = [ordered]@{ passed=$ps51Total; total=28 }
        psscriptanalyzer_findings = $findings.Count
        python_syntax = 'passed'
        ordinal_metadata_contract = 'passed'
    }
    l0_binding = [ordered]@{
    ...
    metadata = [ordered]@{
        ...
        ordering_contract = 'utf8_ordinal'
...
Write-NxbPlatformEventV2CertificationJson -Path $receiptPath -InputObject $receipt
...
Write-Information -MessageData "SUPERBLOCK 2 L1 V2 passed: definitions=$($result.event_definition_count) logs=$($result.attached_log_count) sampledA=$($result.baseline_a_sampled_events) sampledB=$($result.baseline_b_sampled_events) metadata=$metadataA" -InformationAction Continue
