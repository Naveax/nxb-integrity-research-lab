...
param(
    ...
    [string]$SchemaPath = (Join-Path `
        ...
        'schemas\trace-loss-accounting.schema.json')
)
...
if (-not (Test-Path -LiteralPath $schemaFull -PathType Leaf)) {
    throw "Trace-loss accounting schema bulunamadı: $schemaFull"
}
...
if (-not $python) {
    throw "Python bulunamadı. Trace-loss accounting doğrulaması için Python ve 'jsonschema' paketi gerekir."
}

$validator = Join-Path `
    (Split-Path -Parent $PSScriptRoot) `
    'tools\validate_trace_loss_accounting.py'
if (-not (Test-Path -LiteralPath $validator -PathType Leaf)) {
    throw "Trace-loss accounting semantic validator bulunamadı: $validator"
}
...
if ($exitCode -ne 0) {
    throw "Trace-loss accounting doğrulaması başarısız (exit $exitCode):`n$($outputText -join [Environment]::NewLine)"
}
